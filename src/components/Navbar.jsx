...
function Navbar() {
  ...
  return (
    <div className={`w-full px-16 py-8 font-['Neue Montreal'] flex justify-between item-center bg-[#F1F1F1] text-[#212121] fixed z-10 ${visible ? '' : 'hidden'}`}>
      ...
      <div className='Links flex gap-10'>
        {["Services", "Our Work", "About Us", "Insights", "Contact Us"].map((item, index) => (
          <a key={index} className={`text-xl capitalize font-Light ${index === 4 && "ml-56"}`}>{item}</a>
        ))}
      </div>
    </div>
  );
